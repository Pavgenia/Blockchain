# Проект по блокчейну для логистической компании
# Правдина, Морозовский, Кирьянова 15.11Д-БИЦТ09/21б
...
# Эта функция ниже создана для создания самого первого блока и установки его хэша равным "0"
    ...
    def postgre ():
        conn = psycopg2.connect(dbname = 'logistics', user='postgres', password='84848', host='localhost')
        cursor = conn.cursor()
...
# Эта функция ниже создана для добавления дополнительных блоков в цепочку
    def create_block(self, proof, previous_hash):
        ...
        block = {'index': len(self.chain) + 1,
                 ...
                 'phone number': str(df[2].iloc[len(self.chain)]),
                 'address B': str(df[3].iloc[len(self.chain)]),
                 'address A': str(df[4].iloc[len(self.chain)]),
                 'proof': proof,
